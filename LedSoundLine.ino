...
#define PATTERN_BUTTON_PIN 26
#define COLOR_BUTTON_PIN 28
//#define LED_PIN 13
#define LED_STRIP_PIN 24
#define PARM_POT A8
...
uint8_t mode = 5;
uint8_t pattern = PATTERN_RANDOM;
uint8_t cutoffFreqBand = 6;
...
uint8_t ledConfig = LEDCONFIG_300;
uint16_t parm = 1024;
uint16_t lastParm;
...
uint8_t noiseThreshold, *data, nBins, binNum, weighting, c;
uint16_t i, j;
...
void colorFillWipe2(uint32_t c, int wait) {
...
void setup() {
  ...
  lcd.createChar(0, lcdch0);
  lcd.createChar(1, lcdch1);
  lcd.createChar(2, lcdch2);
  lcd.createChar(3, lcdch3);
  lcd.createChar(4, lcdch4);
  lcd.createChar(5, lcdch5);
  lcd.createChar(6, lcdch6);
  lcd.createChar(7, lcdch7);
  lcd.begin(16, 2);
  lcd.setCursor(0, 0);
  lcd.print("      LED       ");
  lcd.setCursor(0, 1);
...
void showSpectre(uint16_t * spectre) {
#ifdef LCD
  lcd.setCursor(0, 1);
  for (int i=1 ; i<17 ; i++) {
    //lcd.write(byte(spectre[i*2]/9400));
    lcd.write(byte(spectre[i*2]/6));
  }
#endif
}
...
//  lcd.setCursor(0, 0);
//  lcd.print("F");
//  lcd.print(cutoffFreqBand+1);
//  lcd.print("P");
//  lcd.print(pattern);
//  lcd.print("A");
//  lcd.write(byte(parm/128));
//  lcd.print(" B");
//  for (i = 0; i <= cutoffFreqBand; i++) {
//    lcd.write(byte(bandPeakLevel[i]/2));
//  }
//#endif
#ifdef LCD
lcd.setCursor(0, 0);
...
switch (pattern) {
    case PATTERN_DANCE_PARTY:
      lcd.print("DANCE PARTY");
      break;
    case PATTERN_PULSE:
      lcd.print("PULSE      ");
      break;
    case PATTERN_SINGLE_DIR_DANCE_PARTY:
      lcd.print("S DIR DANCE");
      break;
...
//  Serial.println(pos);
  ...
  if (pattern == PATTERN_PULSE) {
    ...
    ///colorFillWipe2(strip.Color(255, 255, 255), 500); // Red
    return;
  }
